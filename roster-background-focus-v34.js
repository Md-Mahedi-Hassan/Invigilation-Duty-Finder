...
function searchHasTypedValue() {
  return Boolean(searchInput && String(searchInput.value || "").trim().length);
}

function playVideoQuietly() {
  if (!backgroundVideo || prefersReducedMotion?.matches || document.hidden || document.body.classList.contains("video-fallback-mode")) return;
  const playResult = backgroundVideo.play();
  if (playResult && typeof playResult.catch === "function") playResult.catch(() => {});
}

function updateRosterBackgroundMode() {
  const focused = personRosterIsVisible() || searchHasTypedValue();
  document.body.classList.toggle("roster-focus-mode", focused);

  if (!backgroundVideo) return;
  if (focused || document.body.classList.contains("video-fallback-mode")) {
